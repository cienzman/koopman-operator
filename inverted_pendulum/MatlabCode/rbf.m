...
type = lower(type); % Imposta il tipo di funzione RBF in lettere minuscole
if(~exist('eps','var') || isempty(eps))    % Imposta il valore predefinito di eps se non specificato
    eps = 1;
end
...
Cbig = C; %copio C in Cbih
Y = zeros(size(C,2),size(X,2));
for i = 1:size(Cbig,2)
    C = Cbig(:,i);
    C = repmat( C,1, size(X,4) );
    r_squared = sum( (X - C).^2 ); %Calcola la distanza quadratica tra X e il centro C
    switch type
        case 'thinplate'
